...
前端，然后以低 CPU 优先级、限制并发并串行构建 CentOS 7 / glibc 2.17 Linux AMD64
Server 和 macOS ARM Client，避免构建机上的 Client 心跳被本机编译压力挤掉。
...
Client 的公网 Tunnel 会先进入消息循环，再异步处理一次初始 inventory；已有 durable
outbox 时直接续传而不重复生成全量 inventory，避免积压与重连互相放大。
...
新 Client 启动时会先对全部 active/recovering 会话执行一次恢复；暂时失败的会话保留为
`recovering` 并在后台重试，本地 HTTP 和公网连接不会被无限阻塞。Agent Host 自身只在
provider 全部空闲后轮换版本，关闭超时则退出并由 launchd 重新拉起。
...
每个新会话可在页面选择 Codex（默认）、Kimi 或 Pi。Kimi 默认由 Agent Host 通过 `kimi web --no-open --port 58627` 启动，地址可用 `kimi_server_url` 配置，命令和参数可用 `kimi_command`、`kimi_args` 覆盖；地址必须保持为 loopback HTTP。Pi 默认通过 `pi --mode rpc` 按会话启动，可用 `pi_command`、`pi_args` 覆盖。Client 更新时 Kimi 进程和正在生成的会话保持运行，新的 Client 会重新订阅并核对会话状态。

macOS 后台服务会记住安装时的 `PATH`，但裸命令不会因此固定到旧的 NVM Node 目录：当 `PATH` 指向某个 NVM 版本而 `~/.nvm/alias/default` 已切换时，Client 会在每次启动 provider 时优先使用当前 default 版本中的同名命令。显式配置的绝对路径或 `PATH` 中非 NVM 的稳定 shim 仍保持最高优先级。因此后续升级 Pi/Codex 或切换 NVM default 无需重装 LaunchAgent；新进程自动使用新版本，已经运行的 provider 进程不受影响。
