...
export function ProjectPage({ projectId }: { projectId: string }) {
  ...
  const selectedProvider = info.data?.providers?.find((item) => item.provider === provider);
  const providerAvailable = selectedProvider?.available ?? provider === "codex";
  const canCreate = Boolean(
    !unassigned &&
      (info.data?.providers?.some((item) => item.available) ?? info.data?.appServerRunning),
  );
    ...
    <div className="page">
      ...
      <div className="page-scroll">
        <div className="page-col">
          {!canCreate && !info.isLoading && !unassigned ? (
            <div className="notice-banner warn compact">本机没有可用的执行引擎</div>
          ) : null}
          {threads.isLoading ? (
